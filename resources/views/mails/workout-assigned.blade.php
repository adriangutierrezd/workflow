...
<head>
    ...
    <style>
        ...
        *,
        ...
        :is(.dark section.mail > main > p) {
        --tw-text-opacity: 1;
        color: rgb(var(--tw-color-gray-300) / var(--tw-text-opacity));
        }

        section.mail > main > p > span {
...
<body>
    <section class="mail">
        ...
        <main>
            ...
            <p>
                {{$trainer->name}} {{__('has assigned you a new workout')}}: <span>{{ $workout->title }}</span>
            </p>
            
            <a target="_blank" href="{{route('workouts.edit', $workout->id)}}">
                {{__('See workout')}}
            </a>
        </main>
